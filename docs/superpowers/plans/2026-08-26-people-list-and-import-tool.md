...
### Task 16: System-wide Duplicates tab with employee merge

**Files:**
- Modify: `prisma/schema.prisma` — add `model DuplicateDismissal { id String @id @default(uuid()); employeeAId String; employeeBId String; dismissedById String; createdAt DateTime @default(now()); @@unique([employeeAId, employeeBId]) }` (plain ids, no relations), then `db push` + `generate`.
- Modify: `src/lib/import-export/duplicates.ts` — `detectDuplicates(rows, employees, opts?: { pairEmployees?: boolean; dismissedPairs?: Set<string> })`; `pairKey(a: string, b: string)` = sorted ids joined `|`; when `pairEmployees` is false (default) keep today's behaviour; when true employee↔employee buckets link too, unless `dismissedPairs` has their key. Add tests.
- Create: `src/lib/import-export/employee-fk-targets.ts` — `EMPLOYEE_FK_TARGETS: { model: string; delegate: string; field: string; nullable: boolean }[]` (49 entries; the list printed below) + test `__tests__/employee-fk-targets.test.ts` that parses `prisma/schema.prisma` with the regex approach and asserts the list matches exactly (so a new relation fails the test).
- Create: `src/lib/import-export/employee-merge-service.ts` (server) — `mergeEmployees(args: { primaryId; duplicateIds; data: RowData; actorUserId }): Promise<{ moved: Record<string, number>; detachedUsers: string[]; deleted: string[] }>` per spec §6. Reassign with `(db as any)[delegate].updateMany({ where: { [field]: dupId }, data: { [field]: primaryId } })`; on Prisma error code `P2002` fall back to row-by-row (`findMany` → for each `update`; on `P2002` `delete` that row). Handle `User.employeeId` specially (relink vs detach). Then `db.employee.delete({ where: { id: dupId } })`. Apply `data` to the primary using the same rules as an import UPDATE (reuse the helper from `commit-service.ts` if it exports one; otherwise write `employeeUpdateFromRowData` in `src/lib/import-export/employee-write.ts` and have both use it).
- Create: `src/lib/actions/duplicates.ts` (`"use server"`): `scanSystemDuplicates(): Promise<{ groups: SystemGroup[]; employees: Record<string, EmployeeSnapshot> }>` (`SystemGroup = { id: string; reasons: GroupReason[]; members: MemberRef[] }`, id = group key), `mergeSystemGroup(primaryId, duplicateIds, choices, overrides)` (build result data with `buildMergePlan`-like logic over employee snapshots — write `mergeEmployeeData(members, primary, choices, overrides)` in `merge.ts` reusing `defaultFieldChoices`; validate with `validateRow`), `dismissSystemGroup(memberIds: string[])`. All require SUPER_ADMIN/ADMIN/HR.
- Refactor: `src/components/data/compare-panel.tsx` → extract the presentational table + merge-mode state into `src/components/data/compare-table.tsx` (`CompareTable({ members: PanelMember[], groupId, reasons, readOnly, busy, merging, primary, choices, overrides, onChoosePrimary, onChooseField, onEdit, onReset })`) and keep `ComparePanel` as the import-specific wrapper with unchanged behaviour. Create `src/components/data/system-duplicates-view.tsx` (`"use client"`): Scan now button, group list (left), compare (right) with Merge into one / Not duplicates / Cancel / Confirm; confirm `Dialog` names the records that will be deleted and any login that will be detached.
- Modify: `src/app/(dashboard)/data/page.tsx` — add the **Duplicates** tab (`?tab=duplicates`) rendering `<SystemDuplicatesView />` (client component that calls `scanSystemDuplicates` on mount and on Scan now).

Employee foreign keys (model.field, `?` = nullable):
Department.headId? · Employee.managerId? · Employee.buddyId? · User.employeeId? (unique) · ChecklistItem.assigneeId? · EmployeeTask.employeeId · EmployeeTask.assigneeId? · EmployeeTask.completedById? · ReviewCycle.employeeId? · Review.employeeId · Review.reviewerId · FeedPost.authorId · FeedPost.mentionedEmployeeId? · FeedPollVote.employeeId · FeedComment.authorId · FeedReaction.employeeId · EmergencyAlert.sentById · Notification.recipientId · NotificationRecipient.employeeId · Document.employeeId · HRNote.employeeId · HRNote.authorId · Candidate.managerId? · Interview.interviewerId? · SigningRequest.employeeId? · SigningRequest.countersignerId? · OutOfOffice.employeeId · TrainingGroup.createdById · TrainingGroupMember.employeeId · TrainingClass.organizerId · TrainingClass.createdById · TimeOffBalance.employeeId · TimeOffRequest.employeeId · TimeOffRequest.approverId? · ClubMember.employeeId · PulseResponse.employeeId · StageDocument.countersignerId? · PositionDocument.countersignerId? · EmailDelivery.senderEmployeeId? · ChatMember.employeeId · Channel.createdById · ChannelMember.employeeId · Message.authorId · DmMember.employeeId · Reaction.employeeId · PinnedMessage.pinnedById · SavedMessage.employeeId · OneOnOne.employeeId · OneOnOne.managerId

Delegates are the lowerCamel model names (`db.feedPollVote`, `db.oneOnOne`, …). Skip `Employee.managerId`/`buddyId` rows where the row *is* a duplicate being deleted.
