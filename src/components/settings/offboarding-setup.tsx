...
function getDueDayLabel(dueDay: number | null): string {
  ...
  if (dueDay === 1) return "Last day";
  if (dueDay % 7 === 0) {
    const weeks = dueDay / 7;
    return `${weeks} week${weeks > 1 ? "s" : ""} after`;
  }
  return `${dueDay} day${dueDay > 1 ? "s" : ""} after`;
}
...
export function OffboardingSetup({
  departments,
  employees,
  jobTitles,
}: {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        const lists = await getChecklistsForDepartment(deptId, "OFFBOARDING");
        if (cancelled) return;
        setChecklists(lists.map(mapChecklist));

        if (selectedDeptId) {
          const ovr = await getOverridesForDepartment(selectedDeptId, "OFFBOARDING");
          if (cancelled) return;
  ...
  function refreshData() {
    ...
    // Re-fetch local data
    const deptId = selectedDeptId || null;
    getChecklistsForDepartment(deptId, "OFFBOARDING").then((lists) => {
      setChecklists(lists.map(mapChecklist));
    });
    if (selectedDeptId) {
      getOverridesForDepartment(selectedDeptId, "OFFBOARDING").then((ovr) => {
        setOverrides(ovr.map(mapOverride));
      });
    }
  }
  ...
  async function handleCreateChecklist() {
    ...
    const name = selectedDeptId
      ? `${selectedDeptName} Offboarding`
      : "Global Offboarding";
    await createChecklist(name, "OFFBOARDING", selectedDeptId || undefined);
            ...
            {checklists.length === 0 && (
              <div className="text-center py-8">
                <p className="text-sm text-[var(--color-text-muted)] mb-3">
                  No offboarding checklist for {selectedDeptName}. Create one to get started.
                </p>
                <button
